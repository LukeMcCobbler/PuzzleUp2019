            ...
            new Combinatorics().GenerateCombinations(digits, 10, result => { return; }, (addCandidate, position, partialSolution) =>
                   {
                       ...
                       if (position >= 2 && !checkVowelStarters(partialSolution.Skip(2 - position).Take(2).Concat(candidate), vowelStarters))
                       {
                           return false;
                       }
                       if (position >= 1 && !checkConsonantEnders(partialSolution.Skip(1 - position).Take(1).Concat(candidate), consonantEnders))
                       {
                           return false;
                       }
